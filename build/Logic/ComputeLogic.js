...
exports.traductToAlgoElements = exports.ComputeLogic = void 0;
const DEBUG = false;
function getMatchIndexes(str, re) {
    let indexMatches = [];
    let match;
    if (typeof re == "string")
        re = new RegExp(`[${re}]`, "g");
...
class AlgoElement {
    ...
    logic;
    value;
    constructor(elementA, elementB, logic) {
...
class ComputeLogic {
    ...
    description;
    static logics = [];
    constructor(separator, call, description) {
...
exports.ComputeLogic = ComputeLogic;
const paranthesesSeparator = {
    start: "",
    end: ""
};
...
const divide = new ComputeLogic("/", (x, y) => {
    if (DEBUG)
        console.warn(x, "/", y, "=", x / y);
    if (y === 0)
        throw new Error("Non divisible par 0");
    return x / y;
}, "Divise deux nombres ");
...
const square = new ComputeLogic({ start: "sqrt", end: "" }, (x, y) => {
    if (DEBUG)
        console.warn('sqrt(', x, ")=", Math.sqrt(x), " \t Y = ", y);
    if (x < 0)
        throw new Error("Impossible de faire la racine d'une nombre négatif");
    return Math.sqrt(x);
...
function matchingBetweenSeparator(separator, isSimple = false) {
...
function testEntry(value) {
    ...
    let complexType = Object.values(logicPriorities).flatMap(x => x).filter(x => typeof x.separator != "string")
        .map(x => {
        if (typeof x.separator == "string")
            return;
        return Object.values(x.separator);
    }).flatMap(x => x).join("");
    if (new RegExp(`[^${basicTypeStringRegex}\\(\\) \\d${complexType}]`, "g").test(value))
        throw new Error("Saisie incorrect");
    let opening = getMatchIndexes(value, '(').length;
    let closing = getMatchIndexes(value, ')').length;
    if (opening != closing)
        throw new Error("Saisie incorrect");
}
function traductToAlgoElements(value, isChild = false, childNumber = 0, algoElements = []) {
    // if (isChild) console.log(`${"".padStart(childNumber, "\t")}Child(${childNumber}) to process : ${value}`)
    // else console.log(`to process : ${value}`)
    if (!isChild)
        testEntry(value);
    if (!isNaN(Number(value)))
        return new AlgoElement(Number(value), 0, parenthesis);
        ...
        for (const logic of logics) {
            ...
            if (typeof logic.separator == "string") {
                potentialIndex.forEach(index => {
                    if (!calculationOrder[logicPrio])
                        calculationOrder[logicPrio] = [];
    ...
    for (const logic of calculationOrderArray) {
        ...
        if (!elementToProcess)
            throw new Error("Impossible d'avoir un objet à ne pas prendre.");
        /** Je dois faire un truc la mais quoi :') */
        if (typeof logic.separator != "string")
            return;
        const operationString = elementToProcess.split(logic.separator);
        const numOrAlgA = operationString[1] && indexRegex.test(operationString[0]) ?
            algoElements[Number(operationString[0].replace("[index=", "").replace(']', ""))]
            :
                Number(operationString[0]);
        const numOrAlgB = operationString[1] && indexRegex.test(operationString[1]) ?
    ...
    if (algoElements.length == 0)
        throw new Error("Algo element vide");
    // if (isChild) console.log(`${"".padStart(childNumber, "\t")}EndChild(${childNumber})`)
    return algoElements.pop();
}
exports.traductToAlgoElements = traductToAlgoElements;
//# sourceMappingURL=ComputeLogic.js.map