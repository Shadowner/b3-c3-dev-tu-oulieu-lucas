import { traductToAlgoElements, ComputeLogic } from './Logic/ComputeLogic';
const prompt = require("prompt-sync")({ sigint: true });
let end = false;
let hasPromptedList = false;
do {
    if (!hasPromptedList) console.clear();
    const algo = prompt("Merci d'entrer votre équation ou 'liste' pour la liste des opérations possibles : ");
    if (algo == "") continue;
    if (algo == "liste") {
        hasPromptedList = true;
        let toShown = ''
        ComputeLogic.logics.forEach(logic => {
            if (!logic.description) return;
        ...
        if (hasPromptedList) hasPromptedList = false;
        console.clear();
        console.log("Calcule en cours.");
        try {
            const res = traductToAlgoElements(algo);
